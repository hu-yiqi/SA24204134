...
vignette: >
  %\VignetteIndexEntry{SA24204134_HW}
...
In this example, we perform a two-sample t-test to compare the means of two groups. The null hypothesis is that there is no significant difference between the groups:

\[
H_0: \mu_1 = \mu_2
\]

where \( \mu_1 \) and \( \mu_2 \) are the means of Group 1 and Group 2, respectively.
...
These examples demonstrate how to use `knitr` to create dynamic reports that mix text, code, tables, figures, and mathematical formulas in a single document.


## Demostration 2

In this example, we analyze a simple linear regression model to understand the relationship between two variables: `x` and `y`. The model is defined as:
...
## Demostration 3
The table below shows the descriptive statistics for the variables Sepal.Length, Sepal.Width, Petal.Length, and Petal.Width from the iris dataset.
...
# Hw_1
## Question 3.4
- 本题采用逆变换法构造Rayleigh分布随机数，一共10000个。
- 从生成的随机数频率直方图和Rayleigh分布密度函数图象对比可以看出高度吻合。
```{r}
# 设置随机种子。
set.seed(123)
...
## Question 3.11
 - 以下是对P_1分别取0，0.25，0.50，0.75，1时生成的随机数频率直方图。
 - 从不同P_1取值情况来看，当0 < P_1 < 1时，可以猜想经验分布函数图象具有双峰。
```{r}
set.seed(123)
n <- 1e4
u1 <- rnorm(n)
...
- 定义生成随机数的函数。个数n=10000，时刻t=10。

- 采用均值、方差理论值和估计值的比值来评估生成效果，越接近于1效果越好。
...
- 检验效果，设定不同参数，比较X_t均值，方差的估计值和理论值的比值。
- 从得到的结果来看，效果良好。
...
我们可以找到两个important function：
$$f_1=\frac{1}{\sqrt{2 \pi}*p_1} e^{-x^2 / 2}, \quad x>1$$
其中$p_1$是标准正态分布随机变量大于1的概率。

$$f_2=\frac{x^2}{2*p_2} e^{-x^2 / 2}, \quad x>1$$
其中$p_2$是参数为1的瑞利分布随机变量大于1的概率。
...
```
从图象可以看出，用标准正态分布函数来作为重要性函数，其方差减小效果比瑞利分布要好。

从实际随机数试验结果来看，标准正态分布作为重要性函数，估计方差为0.06861，
远小于瑞利分布函数作为重要性函数，得到的估计两方差36.33402。

但是两种重要性函数得到的估计值都是无偏的，与积分真实值0.40062相差不大。


## Question Monte Calro Experient

For $n=10^4, 2 \times 10^4, 4 \times 10^4, 6 \times 10^4, 8 \times 10^4$, apply the fast sorting algorithm to randomly permuted numbers of $1, \ldots, n$. Calculate computation time averaged over 100 simulations, denoted by $a_n$.
...
```
- 从回归直线图象来看，a_n和t_n高度线性相关。
- 从summary来看，系数通过了t检验，而且R-squared:  0.9993，线性高度拟合。
...
```
FWER: Bonferroni 校正控制 FWER 更好（0.0873），而 B-H 校正的 FWER 较高（0.9305），说明 Bonferroni 方法在控制错误拒绝零假设方面更有效。

FDR: Bonferroni 的 FDR 非常低（0.0045），这意味着在拒绝的假设中几乎没有错误，而 B-H 方法的 FDR 较高（0.0950），显示出 B-H 方法在发现真阳性时允许更多错误。

TPR: B-H 校正的 TPR 更高（0.5611），说明它在识别备择假设方面更有效，而 Bonferroni 方法的 TPR 较低（0.3967）。
...
```
标准正态法置信区间: 该方法基于中心极限定理，假设抽样分布是正态的，但在小样本情况下可能不准确。

基本法置信区间: 基本法计算原始样本和自助样本的差异，通常在分布不对称时表现较好。

百分位法置信区间: 直接基于自助样本的分位数，简单易用，但对极端值敏感。

BCa 方法: 经过偏差校正和加速的置信区间，通常能提供更精确的结果，尤其在样本分布不对称时。
...
```
由t分布的pdf有11.4 Sk-1(a)、Sk(a)相交等价于11.5 equation成立，a是A(k)的横坐标
...
根据生成的 qqplot 和 microbenchmark 的输出，进行以下分析：
	1.	分布相似性：
	•	如果 Q-Q 图接近对角线，说明两种方法生成的随机数分布接近。
	•	如果偏离明显，可能存在实现差异，需要检查自定义函数。
	2.	计算时间：
	•	如果自定义函数的运行时间显著高于默认方法，可以优化实现或使用内置方法。
	•	如果性能相当，则自定义方法在特定情境下可能更灵活。
